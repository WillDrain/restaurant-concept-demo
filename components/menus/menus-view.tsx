...
function TagBadge({ tag }: { tag: string }) {
  return (
    <span className="rounded-full bg-brand-teal/20 px-2 py-0.5 text-xs font-semibold text-brand-teal">
      {tag}
    </span>
  );
}
  ...
  return (
    <li className="py-1.5">
      <div className="flex items-baseline gap-2">
        ...
        <span
          aria-hidden
          className="mx-1 hidden flex-1 translate-y-[-0.2em] border-b border-dotted border-brand-navy/30 sm:block"
        />
        <span className="ml-auto whitespace-nowrap font-semibold tabular-nums text-brand-navy sm:ml-0">
          {formatPrice(item.price)}
        </span>
      </div>
      {item.notes ? (
        <p className="mt-0.5 max-w-prose text-sm leading-snug text-brand-navy/60">
          {item.notes}
        </p>
      ) : null}
    </li>
...
function ModifiersCallout({ group }: { group: Group }) {
  ...
  return (
    <div className="mt-4 rounded-lg border border-brand-teal/20 bg-brand-teal/10 p-4">
      <dl className="space-y-2">
        {group.modifiers.map((modifier) => (
          ...
          >
            ...
            <dd className="font-semibold tabular-nums text-brand-teal">
              {formatPrice(modifier.price)}
            </dd>
            {modifier.options ? (
              <dd className="w-full text-brand-navy/60">{modifier.options}</dd>
            ) : null}
          </div>
        ))}
      </dl>
...
function GroupBlock({ group }: { group: Group }) {
  return (
    <section className="border-t border-brand-teal/15 py-6 first:border-t-0">
      ...
      {group.description ? (
        <p className="mt-1 max-w-prose text-sm italic leading-snug text-brand-navy/70">
          {group.description}
        </p>
      ) : null}
      ...
      {group.footnote ? (
        <p className="mt-3 text-sm italic text-brand-navy/60">{group.footnote}</p>
      ) : null}
    </section>
  );
}
...
function DonutSection({ section }: { section: Section }) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <div className="overflow-hidden rounded-2xl border border-brand-sun/40 bg-brand-sand shadow-sm">
        <div className="bg-brand-teal px-6 py-5 text-center text-brand-cream">
        ...
        <ul className="grid grid-cols-1 gap-x-10 gap-y-1 px-6 py-6 sm:grid-cols-2">
          {(group?.items ?? []).map((flavor) => (
            <li
              key={flavor.name}
              className="flex items-baseline gap-2 border-b border-dotted border-brand-navy/20 py-2"
            >
              <span className="font-semibold text-brand-navy">
                {flavor.name}
              </span>
              <span className="ml-auto font-semibold tabular-nums text-brand-navy">
        ...
        <div className="mx-6 mb-6 rounded-lg border border-brand-sun/40 bg-brand-sun/15 px-4 py-3 text-center">
          <p className="font-display text-lg font-semibold text-brand-ember">
...
function DisclaimersBlock({ disclaimers }: { disclaimers: Disclaimers }) {
  ...
  return (
    <section
      ...
      className="mt-12 border-t border-brand-teal/20 pt-6"
    >
      <ul className="space-y-1.5 text-xs italic leading-relaxed text-brand-navy/60">
        {lines.map((line) => (
          <li key={line}>{line}</li>
        ))}
      </ul>
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-10 sm:px-6 sm:py-14">
      <header className="text-center">
        <h1 className="font-display text-4xl font-semibold text-brand-navy sm:text-5xl">
          Our Menus
        </h1>
      </header>
              ...
              <button
                ...
                className={`whitespace-nowrap rounded-full px-5 py-2 font-display text-sm font-semibold transition-colors sm:text-base ${
                  isActive
                    ...
                    : "bg-brand-sand text-brand-navy hover:bg-brand-sand/70"
                }`}
              >
                {section.title}
              </button>
      ...
      <div className="mt-5 flex justify-center">
        <button
          ...
          className="inline-flex items-center gap-3 rounded-full px-2 py-1 text-sm font-medium text-brand-navy focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-teal focus-visible:ring-offset-2"
        >
          <span
            aria-hidden
            className={`relative inline-block h-6 w-12 shrink-0 rounded-full transition-colors ${
              glutenOnly ? "bg-brand-teal" : "bg-brand-sand"
            }`}
          >
            <span
          ...
          activeSection.featured ? (
            ...
            <article>
              {activeSection.subtitle ? (
                <p className="mx-auto max-w-3xl text-center text-sm italic text-brand-navy/70">
                  {activeSection.subtitle}
                </p>
              ) : null}
              {activeSection.description ? (
                <p className="mx-auto mt-2 max-w-3xl text-center text-sm italic text-brand-navy/60">
                  {activeSection.description}
                </p>
              ) : null}
              ...
              {visibleGroups.length > 0 ? (
                ...
                <p className="mt-10 text-center text-brand-navy/60">
                  No gluten friendly items in this section.
                </p>
              )}

              {activeSection.footnote ? (
                <p className="mt-8 text-center text-sm italic text-brand-navy/60">
                  {activeSection.footnote}
                </p>
              ) : null}
            </article>
